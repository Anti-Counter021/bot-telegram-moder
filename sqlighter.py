...
class SQLighter:
    ...
    def exists(self, user_id: int):
        ...
        with self.connection:
            user_exists = bool(
                len(self.cursor.execute("SELECT * FROM warnings WHERE user_id = ?", (user_id,)).fetchall()))
    ...
    def add_user(self, user_id: int):
        ...
        with self.connection:
            ...
            if not self.exists(user_id):
                return self.cursor.execute("INSERT INTO warnings (user_id) VALUES (?);", (user_id,))
            return
    ...
    def get_count_warnings(self, user_id: int):
        ...
        with self.connection:
            return int(
                self.cursor.execute(
                    "SELECT count_warnings FROM warnings WHERE user_id = (?);", (user_id,)
                ).fetchone()[0]
            )
    ...
    def add_new_warning(self, user_id: int, set_warnings=None):
        ...
        with self.connection:
            ...
            return self.cursor.execute(
                "UPDATE warnings SET count_warnings = (?) WHERE user_id = (?);", (count_warnings, user_id,)
            )


if __name__ == '__main__':
    sqLighter = SQLighter('sql.db')
    sqLighter.add_user(5)
    sqLighter.add_new_warning(1)
